...
public class Nehe16_Fog extends Engine {
	@Override
...
//		lightPosBfr = FloatBuffer.wrap(lightPos);
		...
		fogColorBfr = BufferUtils.newFloatBuffer(fogColor.length);
		fogColorBfr.put(fogColor);
		fogColorBfr.rewind();
	...
	private class EngineX implements EngineDrive{
		...
		public void onResourcesRegister(AliasResourceManager<String> reg) {
			reg.texture("Tex", "data/nehe/nehe_texture_crate.bmp");
		}
		@Override
		public void dispose() {}
		@Override
		public EngineOptions onSetupEngine() {
			final EngineOptions opt = new EngineOptions(new String[]{"data/nehe/nehe_texture_crate.bmp"},800,480);
			return opt;
		}
		int i;
		...
		public void onLoadedResourcesCompleted() {
			...
			Engine.setMainScene(new Scene() {
				...
				public void render(float delta) {
					...
					// update fog
					gl.glFogf(GL10.GL_FOG_MODE, fogMode[i]);
					// fog
...
//					gl.glBindTexture(GL10.GL_TEXTURE_2D, texturesBuffer.get(sceneState.filter));
					...
					Engine.getDefaultFont().drawMultiLine(Engine.getSpriteBatch(), "Tap to switch fog mode\nFling to rotate",
							300, 320);
