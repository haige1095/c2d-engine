...
public class AnimationSimpleTintTest extends Engine {
	@Override
	...
	private class EngineX implements EngineDrive{
		...
		public void onLoadedResourcesCompleted() {
			...
			Tween.to(sprite1, SpriteAccessor.RGB, 500)
			.ease(Bounce.INOUT)
			.target(0.3f, 0.2f,0.8f)
			.repeat(-1, 200)
			.delay(500)
			.start(Engine.getTweenManager());

			Tween.to(sprite2, SpriteAccessor.RGB, 700)
			.ease(Sine.OUT)
			.target(0,0.3f,-.4f )
			.repeatYoyo(-1, 200)
			.start(Engine.getTweenManager());
			...
			Engine.setMainScene(new Scene() {
				...
				public void render(float delta) {
					...
					Engine.debugInfo("Tint via to change the color of the sprite");
				}
